...
class ESMS:
    ...
    def check_stability(self):
        for student in self.M:
            if self.M[
                student] == '':  # if student s_i is not assigned in M, we check if it forms a blocking pair with all the projects in A(s_i).
                p = self.sp[student][0]  # list of pj's wrt to s_i
            else:
                matched_project = self.M[student]  # get the matched project
                rank_matched_project = self.sp[student][1][
                    matched_project]  # find its rank on s_i's preference list A(s_i)
                p_list = self.sp[student][0]  # list of pj's wrt to s_i      # a copy of A(s_i)
                p = p_list[
                    :rank_matched_project]  # we check all the projects that comes before the assigned project in A(s_i)
